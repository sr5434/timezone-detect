...
describe("POST /coords_to_tz", () => {
    ...
    it("should return correct timezone, test 2", async () => {
      const coords2 = {"lat": 40.372834, "long": -121.757330};
      const response2 = await request(baseURL).post("/coords_to_tz").send(coords2);
    ...
    it("should return correct timezone, test 3", async () => {
      ...
      const response3 = await request(baseURL).post("/coords_to_tz").send(coords3);
